...
use crate::{
    models::{cakupan_x_konsep, kata_asing_x_konsep},
    CheckDuplicateTrait,
};
...
pub struct KonsepToCakupan;

impl Linked for KonsepToCakupan {
    type FromEntity = Entity;

    type ToEntity = cakupan::Entity;

    fn link(&self) -> Vec<sea_orm::LinkDef> {
        vec![
            cakupan_x_konsep::Relation::Konsep.def().rev(),
            cakupan_x_konsep::Relation::Cakupan.def(),
        ]
    }
}
pub struct KonsepToKataAsing;

impl Linked for KonsepToKataAsing {
    type FromEntity = Entity;

    type ToEntity = kata_asing::Entity;

    fn link(&self) -> Vec<sea_orm::LinkDef> {
        vec![
            kata_asing_x_konsep::Relation::Konsep.def().rev(),
            kata_asing_x_konsep::Relation::KataAsing.def(),
        ]
    }
}
